...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:import href="http://expath.org/ns/zip"/>
   ...
   <xsl:template name="impl:zip">
      ...
      <xsl:variable name="zip" as="element(zip:file)">
         <zip:file href="{ $pattern }">
            <zip:entry name="content.xml" method="xml">
               <xsl:sequence select="$content"/>
            </zip:entry>
            <zip:dir name="Pictures">
               <xsl:apply-templates select="$contacts//(photo|map)" mode="impl:zip"/>
            </zip:dir>
            <zip:dir name="META-INF">
               <zip:entry name="manifest.xml" method="xml">
                  <xsl:apply-templates mode="impl:manifest" select="
                      zip:xml-entry(resolve-uri($pattern), 'META-INF/manifest.xml')">
   ...
   <xsl:template match="photo" mode="impl:zip">
      <zip:entry name="{ generate-id(.) }.photo.png" method="base64">
         <xsl:sequence select="data(.)"/>
      </zip:entry>
   </xsl:template>

   <xsl:template match="map" mode="impl:zip">
      <zip:entry name="{ generate-id(.) }.map.gif" method="base64">
         <xsl:sequence select="data(.)"/>
      </zip:entry>
   </xsl:template>
